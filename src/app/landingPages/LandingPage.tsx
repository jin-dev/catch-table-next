...
export default function LandingPage() {
  ...
  const [todayCount, setTodayCount] = useState<string | null>(null);
  const t = translations[lang].landing;

  useEffect(() => {
    fetch('/api/tarot/stats/today')
      .then((res) => res.json())
      .then((data) => setTodayCount(data.todayCount?.toLocaleString() ?? null))
      .catch(() => {});
  }, []);
...
<main className={styles.heroSection}>
        ...
        >
          <p>
            {t.socialProof} <strong>{todayCount ?? t.socialProofCount}</strong> {t.socialProofSuffix}
          </p>
        </motion.div>
